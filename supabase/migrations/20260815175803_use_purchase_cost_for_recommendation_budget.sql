-- Budget affordability now depends on the authenticated user's pantry and on
-- purchasable pack sizes. The request-scoped application layer calculates that
-- value after this RPC returns the bounded hard-constraint candidate pool.
...
comment on function public.get_recommendation_candidates(
  text[], text[], integer, integer, integer, text
) is 'Returns at most 50 fully priced recipes matching non-budget hard constraints. The authenticated request layer applies pantry-aware purchasable-pack affordability.';
