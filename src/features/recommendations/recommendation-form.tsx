...
export function RecommendationForm({ defaults }: RecommendationFormProps) {
  ...
  return (
    <div className="grid items-start gap-6 xl:grid-cols-[22rem_minmax(0,1fr)]">
      <form action={formAction} className="space-y-5 rounded-[1.75rem] bg-card p-5 shadow-[0_18px_45px_-32px_rgba(45,39,27,0.45)] ring-1 ring-border/65 xl:sticky xl:top-24">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-primary">
            Today&apos;s constraints
          </p>
          <h2 className="mt-1 font-display text-3xl font-semibold">What fits right now?</h2>
          <p className="mt-2 text-sm leading-6 text-muted-foreground">
            Edit anything that is different today. Dietary needs saved to your profile stay enforced.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="space-y-2">
            ...
            <Input id="servings" name="servings" type="number" min="1" max="30" defaultValue={defaults.servings} inputMode="numeric" />
            <FieldError errors={state.fieldErrors?.servings} />
          </div>
          <div className="space-y-2">
            <Label htmlFor="maxMinutes">Time available</Label>
            <Input id="maxMinutes" name="maxMinutes" type="number" min="5" max="480" defaultValue={defaults.maxMinutes} inputMode="numeric" />
            <FieldError errors={state.fieldErrors?.maxMinutes} />
          </div>
          <div className="space-y-2">
            <Label htmlFor="mealType">Meal</Label>
            <select id="mealType" name="mealType" defaultValue="" className="h-12 w-full rounded-xl bg-background px-3 text-sm outline-none ring-1 ring-input focus-visible:ring-2 focus-visible:ring-ring">
        ...
        <fieldset>
          ...
          <div className="mt-2 grid grid-cols-2 gap-2">
            {equipmentOptions.map((option) => (
              <label key={option.value} className="flex min-h-11 cursor-pointer items-center gap-2 rounded-xl bg-secondary/55 px-3 text-xs font-semibold has-[:checked]:bg-primary has-[:checked]:text-primary-foreground">
        ...
        <fieldset>
          ...
          <div className="mt-2 grid grid-cols-2 gap-2">
            {dietaryOptions.map((option) => {
              const locked = defaults.dietaryPreferences.includes(option.value);
              return (
                <label key={option.value} className="flex min-h-11 items-center gap-2 rounded-xl bg-secondary/55 px-3 text-xs font-semibold has-[:checked]:bg-primary has-[:checked]:text-primary-foreground">
                  {locked ? <input type="hidden" name="dietaryPreferences" value={option.value} /> : null}
                  <input name={locked ? undefined : "dietaryPreferences"} type="checkbox" value={option.value} defaultChecked={locked} disabled={locked} className="size-4 accent-current" />
                  {option.label}{locked ? " · saved" : ""}
                </label>
              );
            })}
          </div>
