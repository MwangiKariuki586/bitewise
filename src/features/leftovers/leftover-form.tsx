...
interface LeftoverEditItem { id: number; name: string; servings: number; prepared_date: string; expiry_date: string; notes: string | null }
...
export function LeftoverForm({ item, today }: { item: LeftoverEditItem | null; today: string }) {
  const [state, action, pending] = useActionState(saveLeftoverAction, initialActionResult);
  return (
    <form action={action} className="space-y-4" noValidate>
      {item ? <input type="hidden" name="id" value={item.id} /> : null}
      <div className="space-y-2"><Label htmlFor="name">Meal or dish</Label><Input id="name" name="name" defaultValue={item?.name ?? ""} placeholder="For example: bean stew" /><ErrorText errors={state.fieldErrors?.name} /></div>
