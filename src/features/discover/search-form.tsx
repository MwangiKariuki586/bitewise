...
export function DiscoverSearchForm({ input }: DiscoverSearchFormProps) {
  ...
  return (
    <form method="get" action="/discover" className="rounded-[1.75rem] bg-card p-4 shadow-[0_18px_55px_-40px_rgba(24,64,45,0.75)] ring-1 ring-border/55 sm:p-6">
      <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto]">
        <div>
          ...
          <div className="relative">
            ...
            <Input id="discover-query" name="q" defaultValue={input.q} placeholder="Search pilau, chapati, beans..." className="pl-12" />
          </div>
        </div>
        <Button type="submit" className="h-12 px-6">Search recipes</Button>
      </div>

      <details className="group mt-4" open={activeFilters > (input.q ? 1 : 0)}>
        <summary className="flex min-h-11 cursor-pointer list-none items-center gap-2 rounded-xl px-2 text-sm font-bold text-primary outline-none focus-visible:ring-2 focus-visible:ring-ring">
          <SlidersHorizontal className="size-4" aria-hidden="true" />
          Refine results
          {activeFilters ? <span className="rounded-full bg-primary px-2 py-0.5 text-xs text-primary-foreground">{activeFilters}</span> : null}
        </summary>
        <div className="mt-4 grid gap-4 border-t border-border/55 pt-5 sm:grid-cols-2 xl:grid-cols-4">
